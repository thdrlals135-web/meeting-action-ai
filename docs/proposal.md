...
본 PoC를 실제 사내 환경에 도입할 경우, 초기 4주는 기능 확장보다 추출 품질과 운영 안정성을 검증하는 기간으로 설정한다.

### 1주차: 파일럿 운영 및 데이터 적재 안정성 확인

- 광고주 캠페인 회의 일부를 대상으로 transcript JSON 기반 파이프라인을 운영한다.
- 회의별 발화 수, chunk 수, 액션아이템 수가 정상적으로 적재되는지 확인한다.
- 파이프라인 재실행 시 중복 적재가 발생하지 않는지 점검한다.
- KPI:
  - 파이프라인 실행 성공률
  - 회의별 적재 누락 건수
  - 중복 적재 발생 여부
...
- 담당자가 실제 회의 결과와 시스템이 생성한 액션아이템을 비교한다.
- 누락된 액션아이템과 불필요하게 생성된 액션아이템을 분류한다.
- confidence가 낮은 항목의 실제 수정 필요 여부를 확인한다.
- KPI:
  - 액션아이템 누락률
  - 잘못 생성된 액션아이템 비율
  - 담당자 수정률
  - 저신뢰 항목 중 실제 수정 필요 비율

### 3주차: confidence 기준 및 프롬프트 개선

- 1~2주차 검토 결과를 바탕으로 confidence 기준을 조정한다.
- 담당자와 기한이 암묵적으로 표현된 케이스를 추가 수집한다.
- 광고·마케팅 약어 사전과 few-shot 예시를 보완한다.
- KPI:
  - confidence 0.85 미만 항목 비율
  - needs_review 항목 처리 시간
  - 반복 오류 유형 감소율

### 4주차: 운영 전환 판단 및 리포트 작성

- 회의 후 정리 시간 감소 효과를 측정한다.
- 담당자별 액션아이템 쏠림과 반복 이슈 키워드를 분석한다.
- 실제 운영 전환 여부와 추가 개발 우선순위를 결정한다.
- KPI:
  - 회의 후 정리 시간 감소율
  - 액션아이템 누락률 감소율
  - 담당자별 미완료 액션아이템 편차
...
4주 운영 후 다음 기준을 충족하면 실제 운영 확대를 검토한다.

- 회의 후 정리 시간이 기존 대비 50% 이상 감소
- 액션아이템 누락률이 지속적으로 감소
- 저신뢰 항목이 대시보드에서 우선 검토 가능
- 담당자가 source_utterance_ids를 통해 원문 근거를 확인할 수 있음
- 반복 이슈 키워드가 캠페인 운영 개선에 활용 가능